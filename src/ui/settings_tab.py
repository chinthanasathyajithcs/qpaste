...
MODE_OPTIONS: list[tuple[str, str]] = [("Idle Inactivity", "idle"), ("Fixed TTL", "ttl")]
...
class SettingsTab(_BaseFrame):  # type: ignore[misc]
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.mode_var: Optional[Any] = None
    ...
    def _build_ui(self) -> None:
        ...
        tk.Checkbutton(
            row_clear, text="Enable", variable=self.auto_clear_var,
        ...
        self._create_menu(row_clear, self.duration_var, [l for l, _ in DURATION_OPTIONS]).pack(side=tk.LEFT, padx=(0, 8))

        curr_mode = self.config.get("auto_clear_mode", "idle")
        mode_label = next((l for l, m in MODE_OPTIONS if m == curr_mode), "Idle Inactivity")
        self.mode_var = tk.StringVar(value=mode_label)
        self._create_menu(row_clear, self.mode_var, [l for l, _ in MODE_OPTIONS]).pack(side=tk.LEFT)
    ...
    def _on_auto_clear_changed(self) -> None:
        ...
        mode = next((m for l, m in MODE_OPTIONS if l == (self.mode_var.get() if self.mode_var else "")), "idle")
        self.config.set("auto_clear_enabled", bool(self.auto_clear_var.get()))
        self.config.set("auto_clear_seconds", secs)
        self.config.set("auto_clear_mode", mode)
